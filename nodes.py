...
3. If the requested date is not available, in the ***follow-up questions*** suggest the next available date along with available time slots.
3. Identify unavailable blocks from events in the Availability Data.
4. Return available blocks as continuous ranges between business hours, skipping unavailable blocks entirely.
5. Rank the blocks by earliest start time and by fit for requested meeting duration.
6. Generate a natural response with suggestions.
    ...
    if is_followup and last_system_message:
        ...
        if available_slots:
            ...
            if unavailable:
                ...
                unavailable_text = f"{names} unavailable for the requested slot. Please select any of the below available slots:<br><br>"
                sentence = f"The following attendee{' is' if len(names) == 1 else 's are'} unavailable: {', '.join(names)}."
