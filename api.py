...
async def websocket_endpoint(websocket: WebSocket, session_id: str):
    ...
    try:
        ...
        while True:
            initial_state = make_initial_state(current_message)
            ...
            while True:
                try:
                    ...
                    print(f"Client {session_id} disconnected during stream")
                    return
                except Exception as e:
                    print(f"Error during stream for {session_id}: {e}")
                    await websocket.send_json({
                        "type": "error",
            ...
            # 🧠 Send final message from AI (if available)
            try:
                ...
                await websocket.send_json({
                    "type": "error",
                    "message": f"Error retrieving final state: {str(e)}"
                })
                return

            # 🔁 Wait for new input to restart workflow
            try:
                new_input = await websocket.receive_json()
                current_message = new_input.get("message", "")
                if final_state.values.get("need_more_details"):
                    current_message = final_state.values.get("question", "")+ " " +current_message
            except WebSocketDisconnect:
                print(f"Client {session_id} disconnected after workflow")
                return

    except WebSocketDisconnect:
        print(f"Client {session_id} disconnected initially")
    except Exception as e:
